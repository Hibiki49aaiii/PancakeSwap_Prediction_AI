...
- A 2026-08-31 authenticated Alchemy BSC Free tier probe returned chain ID 56 and served `eth_getLogs` for 1- and 10-block ranges, but returned HTTP 400 / JSON-RPC `-32600` for larger ranges with an explicit 10-block tier limit. After `JsonRpcClient` was fixed to preserve a JSON-RPC error carried inside an HTTP 400 response, the real recent bootstrap automatically split the requests and completed a 15-minute, 2,000-block Prediction + Chainlink window: 130 Prediction events, 27 Chainlink events, four replay rounds, and zero duplicate canonical heights. The same measured request shape projects to roughly 30 hours and more than 500,000 RPC requests for three days, so this tier was not promoted to the three-day source.
...
The Alchemy result is likewise tier- and date-specific. It proves bounded correctness after adaptive splitting, not acceptable three-day throughput or archive-history capability. Re-probe after a provider-plan change instead of assuming the 10-block limit still applies.
