...
- `UNKNOWN`: the RPC transport failed after submission may have reached the node. Never automatically resend.
...
- `FAILED`: deterministic RPC rejection or reverted receipt.
...
- A transport-unknown outcome is not retried until reconciliation checks the pending nonce.
...
These rules are intended to prevent a classic double-send failure after an ambiguous RPC timeout.
